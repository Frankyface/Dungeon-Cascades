...
describe('draftOptions — basic offer', () => {
  ...
  it('never offers an owned relic', () => {
    ...
    for (let seed = 0; seed < 50; seed++) {
      const { options } = draftOptions(owned, createRng(seed), 'normal');
...
describe('draftOptions — determinism', () => {
  it('same (owned, rngState, tier) ⇒ identical options and advanced state', () => {
    const a = draftOptions([], createRng(7), 'elite');
    const b = draftOptions([], createRng(7), 'elite');
  ...
  it('advances the RNG state (not returned unchanged)', () => {
    ...
    const { rngState } = draftOptions([], start, 'normal');
    expect(rngState).not.toEqual(start);
  });

  it('a different RNG state can produce a different offer', () => {
    const a = draftOptions([], createRng(1), 'normal').options;
    const b = draftOptions([], createRng(999), 'normal').options;
...
describe('draftOptions — tier weighting', () => {
  it('elite drafts surface more elite-tier relics than normal drafts (same seeds)', () => {
    const isElite = (id: string): boolean => getRelic(id).tier === 'elite';
    let eliteDraftElites = 0;
    let normalDraftElites = 0;
    for (let seed = 0; seed < 300; seed++) {
      eliteDraftElites += draftOptions([], createRng(seed), 'elite').options.filter(isElite).length;
      normalDraftElites += draftOptions([], createRng(seed), 'normal').options.filter(isElite).length;
    }
...
describe('draftOptions — graceful exhaustion', () => {
  it('offers fewer than 3 when the unowned pool is smaller than 3', () => {
    const ownAllButTwo = RELIC_IDS.slice(0, RELIC_IDS.length - 2);
    const { options } = draftOptions(ownAllButTwo, createRng(3), 'normal');
    expect(options).toHaveLength(2);
    expect(new Set(options).size).toBe(2);
  });

  it('offers nothing when everything is owned', () => {
    const { options } = draftOptions([...RELIC_IDS], createRng(3), 'normal');
  ...
  it('holds across many seeds and owned subsets', () => {
    fc.assert(
      fc.property(
        ...
        (seed, owned) => {
          const { options } = draftOptions(owned, createRng(seed), 'normal');
          const expectedSize = Math.min(DRAFT_OPTION_COUNT, RELIC_IDS.length - owned.length);
          ...
          for (const id of options) {
            ...
            expect(RELIC_IDS).toContain(id); // valid
          }
        },
      ),
      { numRuns: 300 },
